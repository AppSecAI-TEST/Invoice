...
public abstract class QYBaseAPI {
    ...
    protected final com.fast.sd4324530.fastweixin.company.api.config.QYAPIConfig config;
     ...
     */
    protected QYBaseAPI(com.fast.sd4324530.fastweixin.company.api.config.QYAPIConfig config) {
        this.config = config;
    }
    ...
    protected BaseResponse executePost(String url, String json, File file) {
        ...
        com.fast.sd4324530.fastweixin.util.BeanUtil.requireNonNull(url, "url is null");
        List<File> files = null;
        if (null != file) {
            files = com.fast.sd4324530.fastweixin.util.CollectionUtil.newArrayList(file);
        }
        //需要传token
    ...
    protected boolean isSuccess(String errCode) {
        return com.fast.sd4324530.fastweixin.api.enums.ResultType.SUCCESS.getCode().toString().equals(errCode);
    }
}
