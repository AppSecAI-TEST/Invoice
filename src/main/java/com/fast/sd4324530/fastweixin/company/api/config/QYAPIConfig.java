...
import com.fast.sd4324530.fastweixin.api.config.ConfigChangeNotice;
import com.fast.sd4324530.fastweixin.api.response.GetTokenResponse;
...
public final class QYAPIConfig extends Observable implements Serializable {
    ...
    public void addHandle(final com.fast.sd4324530.fastweixin.handle.ApiConfigChangeHandle handle) {
        super.addObserver(handle);
    }

    public void removeHandle(final com.fast.sd4324530.fastweixin.handle.ApiConfigChangeHandle handle) {
        ...
        NetWorkCenter.get(url, null, new NetWorkCenter.ResponseCallback() {
            public void onResponse(int resultCode, String resultJson) {
                if (HttpStatus.SC_OK == resultCode) {
                    GetTokenResponse response = com.fast.sd4324530.fastweixin.util.JSONUtil.toBean(resultJson, GetTokenResponse.class);
                    ...
                    // 设置通知点
                    setChanged();
                    notifyObservers(new ConfigChangeNotice(corpid, com.fast.sd4324530.fastweixin.api.config.ChangeType.ACCESS_TOKEN, accessToken));
                }
            }
        });
        tokenRefreshing.set(false);
    ...
    private void initJSToken(final long refreshTime) {
        ...
        NetWorkCenter.get(url, null, new NetWorkCenter.ResponseCallback() {
            ...
            public void onResponse(int resultCode, String resultJson) {
                if (HttpStatus.SC_OK == resultCode) {
                    com.fast.sd4324530.fastweixin.api.response.GetJsApiTicketResponse response = com.fast.sd4324530.fastweixin.util.JSONUtil.toBean(resultJson, com.fast.sd4324530.fastweixin.api.response.GetJsApiTicketResponse.class);
                    ...
                    //设置通知点
                    setChanged();
                    notifyObservers(new ConfigChangeNotice(corpid, com.fast.sd4324530.fastweixin.api.config.ChangeType.JS_TOKEN, jsApiTicket));
                }
            }
        });
        jsRefreshing.set(false);
